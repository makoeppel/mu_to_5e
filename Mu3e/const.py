...
Here are defined the most common constants.
...
muon_decay_rate = 2.99598e-16  # MeV
...
def get_error(S, T):
    ...
    eps_pred_err2 = err_T**2 * (S / T**2) ** 2 + err_S**2 * (1 / T**2)
    delta_S = np.sqrt(eps_pred_err2 * T**2 + err_T**2 * (eps) ** 2)

    return delta_S / S, np.sqrt(eps_pred_err2) / eps


def get_pred_w_error(w, mask):
    S = w[mask].sum()
    N = w.sum()

    S_pred_err2 = (w[mask] ** 2).sum()
    N_pred_err2 = (w**2).sum()

    eps = S / N

    eps_pred_err2 = N_pred_err2 * (S / N**2) ** 2 + S_pred_err2 * (1 / N**2)
    delta_S = np.sqrt(eps_pred_err2 * N**2 + N_pred_err2 * (eps) ** 2)
