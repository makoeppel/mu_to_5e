...
BR_mu3e2nu = (Gamma_mu3e2nu / u.MeV) / (const.muon_decay_rate / u.MeV)
...
BR_mu5e2nu = (Gamma_mu5e2nu / u.MeV) / (const.muon_decay_rate / u.MeV)
...
def get_P_and_id_from_MG5(filename):
    ...
    P_grid = (
        np.transpose(P_grid, axes=(1, 0, 2)) * const.MeV_to_GeV
...
class Process:
    ...
    def __init__(
        ...
        model=None,
        n_events=1000,
        import_from_mg5=False,
        mg5_cut_eff=1.0,
        volumetric_sampling=False,
    ):
        """
        ...
        if self.import_from_mg5:
            self.amp = None
        self.volumetric_sampling = volumetric_sampling
    ...
    def initialize_amplitude(self):
        if self.channel != "mu5e" and self.amp is not None:
            ...
            self.nhel = 0  # summ over all helicites -- TODO include polarization
    ...
    def generate(self, force_Emiss=None):
        ...
        elif self.channel == "mu3e2nu_hnl":
            # ------------------------------------------------------
            # µ -> e+ v (N -> v (aprime -> e+ e-))
            aprime = GenParticle("dp_1", self.model.maprime).set_children(
                electron_1, positron_1
            )
            hnl = GenParticle("N", self.model.mN).set_children(nue, aprime)
            self.decay_chain = GenParticle("mu+", MPARENT).set_children(
                positron_0, hnl, numubar
            )
    ...
    def place_it_in_Mu3e(self):
        self.pos, self.w_target = fm.get_decay_positions_in_target(
            self.n_events, volume=self.volumetric_sampling
        )
        ...
        self.total_inv_mass = np.sqrt(
            self.p_total[:, 0] ** 2 - np.sum(self.p_total[:, 1:] ** 2, axis=1)
        )
